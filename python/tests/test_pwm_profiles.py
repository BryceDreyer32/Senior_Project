...
for test_idx in range(0,4):
    ...
    addr = Constants.Constants.PWM_PROFILE0_ADDR
    profile = test_profile[test_idx]
    for point in profile:
        print("writing value" + str(point) + " to address " + str(addr))
    ...
    results = []
    for addr in range(Constants.Constants.ANGLE_CHG_BASE_ADDR, Constants.Constants.ANGLE_CHG_BASE_ADDR + 5):
        results.append(fpga.fpgaRead(addr))

    print("--- RESULTS ---")
    for result in results:
        print(str(result & 0xF) + str((result >> 4) & 0xF))
